...
def listener(sock, *args):
	'''NetiNeti: Asynchronous connection listener. Starts a handler for each connection.'''
...
def handler(conn, *args):
	'''NetiNeti: Asynchronous connection handler. Processes each line from the socket.'''
	global total_data
	data = conn.recv(1024)
	print data
	# time.sleep(5)
